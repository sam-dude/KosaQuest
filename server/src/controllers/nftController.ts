...
type BadgeType = "proverb_apprentice" | "story_master" | "quiz_champion" | "language_explorer";

// Badge types and their requirements
const BADGE_TYPES: Record<BadgeType, {
  name: string;
  description: string;
  xpRequired: number;
  imageUrl: string;
...
export const mintNFTBadge = async (
  ...
  try {
    ...
    const existingBadge = await NFTBadge.findOne({ userId, badgeType: typedBadgeType });
    if (existingBadge) {
